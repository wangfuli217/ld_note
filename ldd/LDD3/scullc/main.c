...
static ssize_t scullc_aio_read(struct kiocb *iocb, const struct iovec *buf, size_t count,
		loff_t pos)
{
	return scullc_defer_op(0, iocb, buf->iov_base, buf->iov_len, pos);
}

static ssize_t scullc_aio_write(struct kiocb *iocb, const struct iovec *buf,
		size_t count, loff_t pos)
{
	return scullc_defer_op(1, iocb, buf->iov_base, buf->iov_len, pos);
}
