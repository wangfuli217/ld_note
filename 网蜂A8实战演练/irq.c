...
/*涉及到中断处理例程，所以就需要注意竟态和并发问题*/
static DECLARE_WAIT_QUEUE_HEAD(button_waitq);

/*唤醒等待队列的条件，可以是任意的布尔表达式*/
...
static irqreturn_t buttons_interrupt(int irq, void *dev_id)
{
	...
	switch(number) {
	/*key0~key5使用的是EINT0～EINT5,看芯片手册就知道是GPN口上的功能*/
	case 0: case 1: case 2: case 3: case 4: case 5:
		tmp = readl(S3C64XX_GPNDAT);
		down = !(tmp & (1<<number));
		break;
		/*key6~key7使用EINT19~EINT20,使用GPL引脚*/
	case 6: case 7:
...
static int s3c64xx_buttons_read(struct file *filp, char __user *buff, size_t count, loff_t *offp)
{
    ...
    if (!ev_press) {
	if (filp->f_flags & O_NONBLOCK)		//非阻塞读取
	    return -EAGAIN;
	else					
	    wait_event_interruptible(button_waitq, ev_press);	//阻塞读取，则进入休眠，等待发生中断时由中断例程唤醒
    }
