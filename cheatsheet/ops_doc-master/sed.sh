...
sed做的 和 我们要做的
    ...
    http://blog.chinaunix.net/uid-20106293-id-142126.html  # sed - 非交互式文本编辑器
...
s/Terrell/Tearrey/' phonelist                                      # 等价 换行分割
    ...
    4.10 sed '/QMPath/{s/QM/PM/ s/=/:/}' config.ini                                                      # 对于同一个区域，还可以使用{}进行处理
...
sed_i_address(){  cat - <<'sed_i_address'
1. 地址: 选择要编辑的行
2. 地址可以是行号或者是上下文地址。
...
sed_class_operation(){ cat - <<'sed_class_operation'
...
-------------------------------------------------------------------------------- d: 表示删除.
       ...
       sed '/^ *#/d; /^ *$/d'     # 删除注释和空白行
       6) sed "/bbo/d" filename   # Remove lines with string (e.g. 'bbo')
       6) sed "/bbo/Id" filename  # Remove lines with string (e.g. 'bbo') # case insensitive:
       7) sed '/^\s*$/d'          # Delete/remove empty lines
       7) sed '/^$/d'             # Delete/remove empty lines
       8) sed -i '$ s/.$//' filename  # Delete/remove last character from end of file
-------------------------------------------------------------------------------- e: 表示多点编辑.
...
#  sed -n -e '/main[[:space:]]*(/,/^}/p' sourcefile.c | more # 打印 C 源文件中的 main() 函数
sed_i_pattern(){ cat - <<'sed_i_pattern'
1. 标准正则表达式
...
sed_out_variable(){ cat - <<'sed_out_variable'
    sed -n ""$a",10p" test.sh
    sed -n ''"$a"',10p' test.sh 
...
sed_out_variable(){ cat - <<'sed_i_misc'
...
sed_i_cmd_map(){ cat - <<'sed_i_cmd_map'
 UNIX          |  SED
 --------------+----------------------------------------------------------------
