...
void CheckBufferOverrun::arrayIndexOutOfBoundsError(const Token *tok, const ArrayInfo &arrayInfo, const std::vector<MathLib::bigint> &index)
{
	...
	reportError(tok, Severity::error, ErrorType::BufferOverrun, "arrayIndexOutOfBounds", oss.str(), ErrorLogger::GenWebIdentity(tok->str()));
}
...
void CheckBufferOverrun::arrayIndexOutOfBoundsError(const Token *tok, const ArrayInfo &arrayInfo, const std::vector<ValueFlow::Value> &index)
{
    ...
    if (condition != nullptr) {
		...
		reportError(callstack, Severity::warning, ErrorType::BufferOverrun, "arrayIndexOutOfBounds", errmsg.str(), 0U, false);
    } else {
        std::ostringstream errmsg;
		...
		reportError(tok, Severity::error, ErrorType::BufferOverrun, "arrayIndexOutOfBounds", errmsg.str(), ErrorLogger::GenWebIdentity(tok->str()));
    }
}
...
void CheckBufferOverrun::arrayIndexOutOfBoundsError(const std::list<const Token *> &callstack, const ArrayInfo &arrayInfo, const std::vector<MathLib::bigint> &index)
{
    std::ostringstream oss;
    makeArrayIndexOutOfBoundsError(oss, arrayInfo, index);
	reportError(callstack, Severity::error, ErrorType::BufferOverrun, "arrayIndexOutOfBounds", oss.str(), 0U, false);
}
...
{
	std::string tmp = varnames;
	reportError(tok, Severity::error, ErrorType::BufferOverrun, "bufferAccessOutOfBounds", bufferOverrunMessage(varnames), 
		ErrorLogger::GenWebIdentity(std::string(tmp.begin(), std::remove(tmp.begin(), tmp.end(), ' '))));
}
...
{
	std::string tmp = varnames;
	reportError(callstack, Severity::error, ErrorType::BufferOverrun, "bufferAccessOutOfBounds", bufferOverrunMessage(varnames),
		0U, false, ErrorLogger::GenWebIdentity(std::string(tmp.begin(), std::remove(tmp.begin(), tmp.end(), ' '))),false);
}

void CheckBufferOverrun::possibleBufferOverrunError(const Token *tok, const std::string &src, const std::string &dst, bool cat)
{
    if (cat)
		reportError(tok, Severity::warning, ErrorType::None, "possibleBufferAccessOutOfBounds",
			...
			ErrorLogger::GenWebIdentity(src+","+dst));
    else
		reportError(tok, Severity::warning, ErrorType::None, "possibleBufferAccessOutOfBounds",
...
void CheckBufferOverrun::strncatUsageError(const Token *tok)
{
	...
	reportError(tok, Severity::error, ErrorType::BufferOverrun, "bufferAccessOutOfBounds",
				...
				"Source: http://www.opensource.apple.com/source/Libc/Libc-167/gen.subproj/i386.subproj/strncat.c", ErrorLogger::GenWebIdentity(tok->str()));
}
...
void CheckBufferOverrun::outOfBoundsError(const Token *tok, const std::string &what, const bool show_size_info, const MathLib::bigint &supplied_size, const MathLib::bigint &actual_size)
{
    ...
    oss << '.';
	reportError(tok, Severity::error, ErrorType::None, "outOfBounds", oss.str(), ErrorLogger::GenWebIdentity(tok->str()));
}
...
void CheckBufferOverrun::pointerOutOfBoundsError(const Token *tok, const Token *index, const MathLib::bigint indexvalue)
{
	...
	reportError(tok, Severity::warning, ErrorType::BufferOverrun, "pointerOutOfBounds", errmsg + ".\n" + verbosemsg, ErrorLogger::GenWebIdentity(tok->str()));
    /*
...
void CheckBufferOverrun::sizeArgumentAsCharError(const Token *tok)
{
    if (_settings && !_settings->isEnabled("warning"))
        return;
	reportError(tok, Severity::warning, ErrorType::None, "sizeArgumentAsChar", "The size argument is given as a char constant.", ErrorLogger::GenWebIdentity(tok->str()));
}


void CheckBufferOverrun::terminateStrncpyError(const Token *tok, const std::string &varname)
{
	reportError(tok, Severity::warning, ErrorType::None, "terminateStrncpy",
                ...
                "assumes buffer is null-terminated.", 0U, true, ErrorLogger::GenWebIdentity(varname));
}

void CheckBufferOverrun::cmdLineArgsError(const Token *tok)
{
	reportError(tok, Severity::error, ErrorType::None, "insecureCmdLineArgs", "Buffer overrun possible for long command line arguments.", ErrorLogger::GenWebIdentity(tok->str()));
}
...
void CheckBufferOverrun::bufferNotZeroTerminatedError(const Token *tok, const std::string &varname, const std::string &function)
{
    ...
    reportError(tok, Severity::warning, ErrorType::None, "bufferNotZeroTerminated", errmsg, 0U, true, ErrorLogger::GenWebIdentity(varname));
}

void CheckBufferOverrun::argumentSizeError(const Token *tok, const std::string &functionName, const std::string &varname)
{
    reportError(tok, Severity::warning, ErrorType::None, "argumentSize", 
		"The array '" + varname + "' is too small, the function '" + functionName + "' expects a bigger one.", ErrorLogger::GenWebIdentity(varname));
}

void CheckBufferOverrun::negativeMemoryAllocationSizeError(const Token *tok)
{
    reportError(tok, Severity::error, ErrorType::None, "negativeMemoryAllocationSize",
                "Memory allocation size is negative.\n"
                "Memory allocation size is negative."
				"Negative allocation size has no specified behaviour.", ErrorLogger::GenWebIdentity(tok->str()));
}
...
static bool checkMinSizes(const std::list<Library::ArgumentChecks::MinSize> &minsizes, const Token * const ftok, const std::size_t arraySize, const Token **charSizeToken, const Settings * const settings)
{
    ...
    for (std::list<Library::ArgumentChecks::MinSize>::const_iterator minsize = minsizes.begin(); minsize != minsizes.end(); ++minsize) {
        ...
        case Library::ArgumentChecks::MinSize::ARGVALUE:
			...
			else if (Token::Match(argtok, "sizeof ("))  // memset(buf, -1, sizeof(buf[0] * 10));
			{
				if (const Token* tok2 = argtok->next()->astOperand2())
				{
					if (tok2->isArithmeticalOp())
					{
						if (4 > arraySize)
							error = true;
					}
			...
			else if (Token::Match(argtok, "%name% ,|)") && argtok->values.size() == 1 && argtok->values.front().isKnown())
			{
				std::size_t buf_len = std::size_t(argtok->values.front().intvalue);
				if (buf_len > arraySize)
				{
					error = true;
				}
			}
...
void CheckBufferOverrun::checkFunctionCall(const Token *tok, const ArrayInfo &arrayInfo, std::list<const Token *> callstack)
{
    ...
    while (argtok) {
		...
		else if (Token::Match(argtok, "%varid% + %var% ,|)", declarationId))
		{
			const ValueFlow::Value* maxValue = argtok->tokAt(2)->getMaxValue(false);
			if (maxValue && maxValue->isKnown())
			{
				const ArrayInfo ai(arrayInfo.limit(maxValue->intvalue));
				checkFunctionParameter(*tok, argnr, ai, callstack);
			}
    ...
    */
    ...
    for (int warn = 0; warn == 0 || warn == 1; ++warn) {
        ...
        for (const Token *tok2 = tok; tok2 && tok2->str() == "["; tok2 = tok2->link()->next()) {
			...
			//eg.arr[-1] not report error
			if (!index || (index->str() == "-1" && !index->isExpandedMacro()))
                continue;
            const ValueFlow::Value *value = index->getValueLE(-1LL,_settings);
			if (value && !index->isNumber())//array[-1] not report Error
...
{
    reportError(tok, Severity::error, ErrorType::None, "negativeArraySize",
		"Declaration of array '" + (tok ? tok->str() : std::string()) + "' with negative size is undefined behaviour", ErrorLogger::GenWebIdentity(tok->str()));
}
            ...
            if (var->isArray()) {
                ...
                for (std::size_t j = 0; j < functions; ++j) {
                    ...
                    for (const Token *tok3 = func_scope->classDef; tok3 && tok3 != func_scope->classEnd; tok3 = tok3->next()) {
                        ...
                        while (tok3 && tok3 != func_scope->classEnd) {//ignore TSC
                            // End of statement.
                            if (tok3->str() == ";") {
                                checkTok = tok3;
                                break;
        ...
        else 
		{
				...
				if (!var)
				{
					...
					if (gtF && !gtF->GetDimensions().empty())
					{
						...
						if (d.IsNum())
						{
							ArrayInfo arrayInfo(tok->varId(), varname, 1U, d.iNum);
							valueFlowCheckArrayIndex(tok->next(), arrayInfo);
						}
					}
					continue;
				...
				else if(var->nameToken() == tok || !var->isArray())
				{
					continue; 
				}
...
MathLib::biguint CheckBufferOverrun::countSprintfLength(const std::string &input_string, const std::list<const Token*> &parameters)
{
    ...
    for (std::string::size_type i = 0; i < input_string.length(); ++i) {
        ...
        if (handleNextParameter) {
            ...
            if (i_d_x_f_found)
                tempDigits = TSC_MAX(static_cast<unsigned int>(tempDigits), 1U);

            if (digits_string.find('.') != std::string::npos) {
                const std::string endStr = digits_string.substr(digits_string.find('.') + 1);
                const unsigned int maxLen = TSC_MAX(static_cast<unsigned int>(std::abs(std::atoi(endStr.c_str()))), 1U);
...
void CheckBufferOverrun::checkBufferAllocatedWithStrlen()
{
    ...
    for (std::size_t i = 0; i < functions; ++i) {
        ...
        for (const Token *tok = scope->classStart->next(); tok && tok != scope->classEnd; tok = tok->next()) {
            ...
            for (const Token* const end = tok->scope()->classEnd; tok && tok->next() && tok != end; tok = tok->next()) {//ignore TSC
...
void CheckBufferOverrun::negativeIndexError(const Token *tok, MathLib::bigint index)
{
	...
	ostr << "Do not use negative variable[" << index << "] as an index to array.";
	reportError(tok, Severity::error, ErrorType::BufferOverrun, "negativeIndex", ostr.str(), ErrorLogger::GenWebIdentity(tok->str()));
}
...
void CheckBufferOverrun::negativeIndexError(const Token *tok, const ValueFlow::Value &index)
{
	...
	ostr << "Do not use negative variable[" << index.intvalue << "] as an index to array.";
    if (index.condition)
        ostr << " Otherwise there is useless condition at line " << index.condition->linenr() << ".";
    reportError(tok, index.condition ? Severity::warning : Severity::error, ErrorType::BufferOverrun, "negativeIndex", ostr.str(), 0U, index.inconclusive);
}
...
{
    const MathLib::bigint uvalue = TSC_MAX(MathLib::bigint(0), value);
    MathLib::bigint n = 1;
        ...
        for (const Token *tok = scope->classStart; tok && tok != scope->classEnd; tok = tok->next()) {
            if (Token::Match(tok, "%name% [ %var% ]")) {
                ...
                if (Token::Match(tok, "&& (| %varid% <|<=", indexID))
                    ...
                    arrayIndexThenCheckError(tok, indexName);
				*/
				if (indexID!=0 && (Token::Match(tok, "&& (| %varid% <|<=", indexID) || Token::Match(tok, "&& (| %any% >|>= %varid% !!+", indexID)))
				{
					//false positive :  if( i!= -1 && a[i] && j<i && b[j])
					if (Token::Match(tok->astOperand1(), "&&"))
					{
						const Token* tmpTok = tok->astOperand1()->astOperand1();
						if (tmpTok && tmpTok->tokType() == Token::eComparisonOp && tmpTok->astOperand1() && tmpTok->astOperand2())
						{
							if (tmpTok->astOperand1()->varId() == indexID || tmpTok->astOperand2()->varId() == indexID)
								continue;
						}
...
{
    reportError(tok, Severity::style, ErrorType::BufferOverrun, "arrayIndexThenCheck",
                ...
                "not be accessed if the index is out of limits.", ErrorLogger::GenWebIdentity(indexName));
}
    ...
    for (std::map<std::string, struct MyFileInfo::ArrayUsage>::const_iterator it = all.arrayUsage.begin(); it != all.arrayUsage.end(); ++it) {
        ...
        if (sz != all.arraySize.end() && sz->second > 0 && sz->second < it->second.index) {
            ...
            ErrorLogger::ErrorMessage errmsg(locationList,
                                                   Severity::error, 
                                                   ostr.str(),
												   ErrorType::BufferOverrun,
                                                   "arrayIndexOutOfBounds",
                                                   false);
			errmsg.SetWebIdentity(ErrorLogger::GenWebIdentity(it->first));
...
static bool GetFuncParamTok(std::list<const Token*>& paramList, const Token *paramtok)
{
...
static int ComputeSizeOfStdType(const Tokenizer *const &tokenizer, const Settings *const &_settings, unsigned int varID)
{
	...
	const Token *typeStartTok = var->typeStartToken();
	const Token *typeEndTok = var->typeEndToken();
	if (typeStartTok == NULL || typeEndTok == NULL)
	{
		return 1;
	}
	...
	if (typeStartTok == typeEndTok)
	{
		if (typeStartTok->isStandardType())
		{
			...
			{
				return _settings->sizeof_bool;
			}
			else if (typeStartTok->str() == "short")
			{
				return _settings->sizeof_short;
			}
			else if (typeStartTok->str() == "int")
			{
				return _settings->sizeof_int;
			}
			else if (typeStartTok->str() == "long")
			{
				...
				{
					return _settings->sizeof_long;
				}
			}
			else if (typeStartTok->str() == "float")
			{
				return _settings->sizeof_float;
			}
			else if (typeStartTok->str() == "double")
			{
				...
				{
					return _settings->sizeof_double;
				}
			}
			else if (typeStartTok->str() == "size_t")
			{
				return _settings->sizeof_size_t;
			}
		}
	}
	// * indicates variable must be pointer type
	else if (typeEndTok->str() == "*")
	{
		return _settings->sizeof_pointer;
	}

	return 1;
}

void CheckBufferOverrun::memcpyLenCheck()
{
	for (const Token *tok = _tokenizer->tokens(); tok; tok = tok->next())
	{
		if (Token::Match(tok, "memcpy ("))
		{
			std::vector<const Token*> params;
			const Token *temptok = tok->tokAt(1);
			getParamsbyAst(temptok, params);
			if (params.size() < 3)
			{
				continue;
			}
			const Token *p1 = params[2];
			const Token *p2 = params[1];
			const Token *p3 = params[0];

			//if(!(p1->isArray()))
			//continue;
			if (!(p1->variable()))
				continue;
			if (!p1->variable()->isArray())
				continue;
			MathLib::bigint p1_size = p1->variable()->dimension(0);
			p1_size *= ComputeSizeOfStdType(this->_tokenizer, this->_settings, p1->varId());

			const ValueFlow::Value *temp = p3->getMaxValue(false);
			if (temp)
			{
				long long p3_max = temp->intvalue;
				if (p3_max > p1_size)
				{
					bufferOverrunError(p1, p1->str());
				}
				if (!p2->variable())
					continue;
				if (!p2->variable()->isArray())
					continue;
				MathLib::bigint p2_size = p2->variable()->dimension(0);
				p2_size *= ComputeSizeOfStdType(_tokenizer, _settings, p2->varId());
				if (p3_max > p2_size)
				{
					bufferOverrunError(p1, p1->str());
				}
...
void CheckBufferOverrun::vsnprintCheck()
{

	for (const Token *tok = _tokenizer->tokens(); tok; tok = tok->next())
	{
		if (Token::Match(tok, "vsnprintf"))
		{
			if (tok->astParent() && tok->astParent()->str() == "(")
			{
				std::list<const Token*> paramsList;
				::GetFuncParamTok(paramsList, tok);

				if (paramsList.size() != 4)
					continue;


				const Token* tokBuf = paramsList.front();

				if (!tokBuf)
					continue;

				const Variable *var = _tokenizer->getSymbolDatabase()->getVariableFromVarId(tokBuf->varId());
				if (var && var->isArray() && var->dimensions().size() == 1)
				{
					const std::size_t len = (std::size_t)var->dimension(0);
					paramsList.pop_front();
					const Token* tokSize = paramsList.front();
					if (!tokSize)
						continue;
					const ValueFlow::Value* pMax = tokSize->getMaxValue(false);
					if (pMax && pMax->varvalue > len)
					{
						std::stringstream ss;
						ss << "[" << tokSize->str() << "] is greater or equal than size of [" << tokBuf->str() << "] which the buffer may be full filled then no space for the terminating null character.";
						reportError(tokSize, Severity::error, ErrorType::BufferOverrun, "bufferAccessOutOfBounds", ss.str(), ErrorLogger::GenWebIdentity(tokBuf->str()));
					}
				}
...
void CheckBufferOverrun::checkSelfAddedSubscript()
{
	...
	for (std::size_t i = 0; i < functions; ++i) {
		...
		for (const Token *tok = scope->classStart; tok && tok != scope->classEnd; tok = tok->next()) {
			if (Token::Match(tok, "%name% [ %var% ]")) {
				...
				{
					if (Token::Match(tmpTok, "%varid% <|<=", indexID) || Token::Match(tmpTok, ">|>= %varid%", indexID))
					{
						break;
					}
					...
					if (Token::Match(tmpTok, "%varid% ++", indexID) || Token::Match(tmpTok, "++ %varid%", indexID))
					{
						...
						{
							if (Token::Match(tmpTok2, "%varid% <|<=", indexID)  || Token::Match(tmpTok2, ">|>= %varid%", indexID))
							{
								if (tmpTok2->next()->str() == "<" && Token::Match(tmpTok2->next()->astOperand2(),"- 1"))
								{
									break;
								}
								else
						...
						if (hasCheck)
						{
							SelfAddedSubscriptError(tmpTok, indexName);
							break;
						}
...
void CheckBufferOverrun::SelfAddedSubscriptError(const Token *tok, const std::string &indexName)
{
