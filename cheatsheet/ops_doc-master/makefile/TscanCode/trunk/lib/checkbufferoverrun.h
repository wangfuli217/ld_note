 ...
 */
class TSCANCODELIB CheckBufferOverrun : public Check {
public:
    ...
    void runSimplifiedChecks(const Tokenizer *tokenizer, const Settings *settings, ErrorLogger *errorLogger)
    {
		...
		checkBufferOverrun.memcpyLenCheck(); //from TSC1.0
		checkBufferOverrun.vsnprintCheck(); //from TSC1.0
...
public:
    void getErrorMessages(ErrorLogger *errorLogger, const Settings *settings) const {
		...
		c.reportError(nullptr, Severity::warning, ErrorType::BufferOverrun, "arrayIndexOutOfBounds", "Array 'x[10]' accessed at index 20, which is out of bounds. Otherwise condition 'y==20' is redundant.", ErrorLogger::GenWebIdentity(""));
    }
private:
