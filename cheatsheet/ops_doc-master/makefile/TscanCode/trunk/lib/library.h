...
public:
	...
	//function exit process or thread or throw
	//temp use
	...
	bool CheckIfLibFunctionReturnNull(const std::string & strFuncName) const;

	bool CheckIfIsLibFunction(const std::string & strFuncName) const;
	...
	bool GetLibNotNullParamIndexByName(std::set<int> &derefIndex, const std::string &strFuncName) const;
