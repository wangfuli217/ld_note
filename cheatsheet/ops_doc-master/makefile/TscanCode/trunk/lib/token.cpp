...
void Token::update_property_info()
{
    if (!_str.empty()) {
        if (_str == "true" || _str == "false")
            ...
            _tokType = eNumber;
        else if (_str.length() > 1 && _str[0] == '"' && *_str.rbegin() == '"')
            _tokType = eString;
        else if (_str.length() > 1 && _str[0] == '\'' && *_str.rbegin() == '\'')
            _tokType = eChar;
...
static int multiComparePercent(const Token *tok, const char*& haystack, bool emptyStringFound, unsigned int varid)
{
    ...
    case 'v':
        if (haystack[3] == '%') { // %var%
            ...
            if (varid == 0) {
                throw InternalError(tok, "Internal error. Token::Match called with varid 0. Please report this to TscanCode developers");
            }
...
void Token::printOut(const char *title, std::ostream& os/* = std::cout*/) const
{
    if (title && title[0])
        os << "\n### " << title << " ###\n";
    os << stringifyList(true, true, true, true, true, 0, 0) << std::endl;
}

void Token::printOut(const char *title, const std::vector<std::string> &fileNames, std::ostream& os/* = std::cout*/) const
{
	...
	os << "\n### Total token count: " << tokCount << " ###\n";
    if (title && title[0])
        os << "\n### " << title << " ###\n";
    os << stringifyList(true, true, true, true, true, &fileNames, 0) << std::endl;
}
...
void Token::stringify(std::ostream& os, bool varid, bool attributes, bool macro) const
{
	...
	if (_function && (_tokType == eFunction))
		os << "@@" << _function;
}
...
const Scope* Token::GetCurrentScope() const
{
...
const Scope* Token::GetFuncScope() const
{
	const Scope *currScope = _scope;
