...
public:
    ...
    void isSigned(bool sign) {//明确写了signed的才是true， int a;没带signed的虽然意思是有符号，但这个接口返回false，特别注意 
     ...
     * for no title.
     */
	void printOut(const char *title = nullptr, std::ostream& os = std::cout) const;
     ...
     */
	void printOut(const char *title, const std::vector<std::string> &fileNames, std::ostream& os = std::cout) const;
...
public:
	...
	bool astUnderTopLeft() const {
		const Token *tok = this;
		const Token* tok2 = tok->astParent();
		while (tok2 && tok2->_astParent)
		{
			tok = tok2;
			tok2 = tok2->astParent();
		}

		return (tok2 && tok2->astOperand1() && tok2->astOperand1() == tok);
	}

	bool astUnderTopRight() const {
		const Token *tok = this;
		const Token* tok2 = tok->astParent();
		while (tok2 && tok2->_astParent)
		{
		...
		return (tok2 && tok2->astOperand2() && tok2->astOperand2() == tok);
	}


	bool astUnderTokenLeft(const Token *pTok) const {
		const Token *tok = this;
		bool bLeft = false;
		while (tok && tok->_astParent && pTok)
		{
			if (tok == pTok->astOperand1()) {
				bLeft = true;
				break;
			 }
			tok = tok->_astParent;
	...
	bool astUnderTokenRight(const Token *pTok) const {
		const Token *tok = this;
		bool bRight = false;
		while (tok && tok->_astParent && pTok)
		{
			if (tok == pTok->astOperand2()) {
				bRight = true;
				break;
			}
			tok = tok->_astParent;
	...
	std::string astString2(const char *sep = "") const {
		std::string ret;
		if (_astOperand1)
			ret = _astOperand1->astString(sep) + sep;
		ret = ret + _str;
		if (_astOperand2)
			ret += sep + _astOperand2->astString(sep);
		return ret;
	}
	...
	const Scope* GetFuncScope() const;
