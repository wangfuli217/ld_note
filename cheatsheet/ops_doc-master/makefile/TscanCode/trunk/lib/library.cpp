...
Library::Error Library::load(const tinyxml2::XMLDocument &doc)
{
    ...
    for (const tinyxml2::XMLElement *node = rootnode->FirstChildElement(); node; node = node->NextSiblingElement()) {
		...
		else if (nodename == "functionexit")
		{
			const char * const name = node->Attribute("name");
			if (!name)
				return Error(MISSING_ATTRIBUTE, "name");
			functionexit.insert(name);
		}
        else
...
Library::Error Library::loadFunction(const tinyxml2::XMLElement * const node, const std::string &name, std::set<std::string> &unknown_elements)
{
    ...
    for (const tinyxml2::XMLElement *functionnode = node->FirstChildElement(); functionnode; functionnode = functionnode->NextSiblingElement()) {
		...
		else if (functionnodename == "retnull")
		{
			functionretnull.insert(name);
		}
		else
...
bool Library::CheckIfLibFunctionReturnNull(const std::string &strFuncName) const
{
	return functionretnull.find(strFuncName) != functionretnull.end();
}

bool Library::GetLibNotNullParamIndexByName(std::set<int> &derefIndex, const std::string& strFuncName) const
{
	...
	bool bLibFunc = false;
	std::map<std::string, std::map<int, Library::ArgumentChecks> >::const_iterator iterLib = argumentChecks.find(strFuncName);
	if (iterLib != argumentChecks.end())
	{
		bLibFunc = true;
		for (std::map<int, Library::ArgumentChecks>::const_iterator iterArg = iterLib->second.begin(), iterArgEnd = iterLib->second.end();
		iterArg != iterArgEnd; ++iterArg)
		{
			if (iterArg->second.notnull)
			{
				//note that param index starts from 1 in cfg files
				derefIndex.insert(iterArg->first - 1);
			}
		}
	}
	return bLibFunc;
...
bool Library::CheckIfIsLibFunction(const std::string &strFuncName) const
{
