...
__base="$(basename "${__file}" .sh)" # basename可以返回文件名(删除扩展名)


# Define the environment variables (and their defaults) that this script depends on
LOG_LEVEL="${LOG_LEVEL:-6}" # 7 = debug -> 0 = emergency                    # 日志级别
NO_COLOR="${NO_COLOR:-}"    # true = disable color. otherwise autodetected  # 颜色信息
...
local colorvar="color_${log_level}"         # 从一个变量映射到另一个变量

local color="${!colorvar:-${color_error}}"  # 默认是错误级别
