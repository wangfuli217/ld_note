...
### debug
# dial_test.-- test_netup_v10 test_netup_v4 test_netup_v6
# dial_test.-- test_apn_3gpp_custom_7 test_apn_3gpp_reserved_1to6 test_apn_3gpp2 test_del_apn
# dial_test.-- _test_twt_dial_apn_test _test_twt_dial_test
#
### environment
...
setUp() {
  ...
  #  if [ ! -e test ]; then
  #    /mnt/mdm9607_qmi/sdk_ubus_dial/build.sh
  #  fi
  #
...
test_netup_v10() {
  ...
  output="$(/usr/local/bin/wm_set_netup_v10 2>&1)"
  status=$?
  ...
  output="$(/usr/local/bin/wm_set_netdown 2>&1)"
  status=$?
...
test_netup_v4() {
  ...
  output="$(/usr/local/bin/wm_set_netup_v4 2>&1)"
  status=$?
...
test_netup_v6() {
  ...
  # prepare
  output="$(/usr/local/bin/wm_set_netup_v6 2>&1)"
  status=$?
...
test_apn_3gpp_custom_7() {
  ...
  while read -r profile_id ip_family user_id apn_name auth_type auth_password; do
    output="$(/usr/local/bin/wm_set_apn_3gpp "${profile_id}" "${ip_family}" "${user_id}" "${apn_name}" "${auth_type}" "${auth_password}")"
    status=$?
...
EOF
  ...
  # restore default apn
  output="$(wm_set_default_apn "${default_apn}")"
  echo "wm_set_default_apn ${default_apn}"
  status=$?
  assertTrue "/usr/local/bin/wm_get_default_apn @ret:${status} @output:${output}" "${status}"
}
...
test_apn_3gpp2() {
  while read -r profile_id ip_family user_id apn_name auth_type auth_password; do
    output="$(/usr/local/bin/wm_set_apn_3gpp2 "${profile_id}" "${ip_family}" "${user_id}" "${apn_name}" "${auth_type}" "${auth_password}")"
    status=$?
...
test_del_apn() {
  ...
  while read -r profile_id ip_family user_id apn_name auth_type auth_password; do
    ...
    output="$(wm_del_apn_profile_id "${profile_id}")"
    status=$?
...
EOF
  ...
  while read apn_name; do
    echo "${apn_name}"
    output="$(wm_set_default_apn "${apn_name}")"
    status=$?
  ...
  output="$(wm_set_default_apn "unconfig_apn")"
  status=$?
  assertTrue "/usr/local/bin/wm_set_default_apn @ret:${status} @output:${output}" "${status}"
...
_test_twt_dial_apn_test() {
  ...
  [ -n "${APN_TEST_CMD}" ] && {
    ...
    while read -r profile_id apn_name ip_family user_id auth_password auth_type; do
      ...
      status=$?
      echo "twt_dial_apn_test -i "${profile_id}" -n "${apn_name}" -f "${ip_family}" -u "${user_id}" -p "${auth_password}" -a "${auth_type}" set"
      assertTrue "twt_dial_apn_test set @ret:${status} @output:${output}" "${status}"
