...
# shunit2:WARN message  -- 级别可能有颜色
# 模块   :级别 内容     -- 级别可能有颜色
# Logging functions.
_shunit_warn() { 
  echo "${__shunit_ansi_yellow}shunit2:WARN${__shunit_ansi_none} $*" >&2
     # "${起始颜色}            模块   : 级别 ${结束颜色}   输出内容" >&2 标志输出到标准错误输出，最终从标准错误输出
}
_shunit_error() {
...
__SHUNIT_UNAME_S=$(uname -s)     # 命令替换
case "${__SHUNIT_UNAME_S}" in    # 根据系统选择命令
  BSD) __SHUNIT_CMD_EXPR='gexpr' ;;
  *) __SHUNIT_CMD_EXPR='expr' ;;
esac

__SHUNIT_CMD_ECHO_ESC='echo -e' # 命令赋值
# shellcheck disable=SC2039
\[ "`echo -e test`" = '-e test' ] && __SHUNIT_CMD_ECHO_ESC='echo' # && 短路执行

# Commands a user can override if needed.
SHUNIT_CMD_EXPR=${SHUNIT_CMD_EXPR:-${__SHUNIT_CMD_EXPR}} # SHUNIT_CMD_EXPR 环境变量未定义，根据环境选择 __SHUNIT_CMD_EXPR

# Enable color output. Options are 'never', 'always', or 'auto'.
SHUNIT_COLOR=${SHUNIT_COLOR:-auto} # SHUNIT_COLOR -> never always auto 

# Specific shell checks. ZSH 检测
...
# Constants
#
__SHUNIT_MODE_SOURCED='sourced'         # sourced方式
__SHUNIT_MODE_STANDALONE='standalone'   # standalone独立执行
__SHUNIT_PARENT=${SHUNIT_PARENT:-$0}    # SHUNIT_PARENT
...
# 使得 assertEquals 等函数支持 --lineno number 选项。在出错时输出具体位置
...
assertEquals(输出字符串和期望字符串相等) {
  # shellcheck disable=SC2090
  ${_SHUNIT_LINENO_}                           # 调用assertEquals的行位置信息
  if \[ $# -lt 2 -o $# -gt 3 ]; then           # 异常判断
    _shunit_error "assertEquals() requires two or three arguments; $# given"
    _shunit_assertFail
    return ${SHUNIT_ERROR}
  fi
  _shunit_shouldSkip && return ${SHUNIT_TRUE}  # 忽略判断

  shunit_message_=${__shunit_lineno}           # __shunit_lineno assertEquals --lineno number 中number信息。
  if \[ $# -eq 3 ]; then
    shunit_message_="${shunit_message_}$1"     # 错误输出提示信息，有行信息包括行信息
    shift
  fi
  shunit_expected_=$1
  shunit_actual_=$2
  ...
  if \[ "${shunit_expected_}" = "${shunit_actual_}" ]; then # 预期和执行结果比较
    _shunit_assertPass
  else
...
# LINENO 当前执行的脚本或shell函数中的行号
_ASSERT_EQUALS_='eval assertEquals --lineno "${LINENO:-}"' # LINENO函数执行所在行； 参数而已
...
assertNotEquals(输出字符串和期望字符串不等) {
  ...
  shunit_return=${SHUNIT_TRUE}
  if \[ "${shunit_expected_}" != "${shunit_actual_}" ]; then # 字符串不相等判断
    _shunit_assertPass
  else
...
# Returns:
#   integer: success (TRUE/FALSE/ERROR constant)
failNotEquals(记录测试失败，统计两只不等并输出) {
  # shellcheck disable=SC2090
  ${_SHUNIT_LINENO_}
  if \[ $# -lt 2 -o $# -gt 3 ]; then
...
# Args:
#   None
startSkipping(强制保留断言，忽略测试函数) { __shunit_skip=${SHUNIT_TRUE}; }

# Resume the normal recording behavior of assert and fail calls.
#
# Args:
#   None
endSkipping(恢复正常记录断言和失败调用) { __shunit_skip=${SHUNIT_FALSE}; }
...
isSkipping(查看是否忽略) { return ${__shunit_skip}; }
...
_shunit_mktempDir(创建临时目录存储各种运行时文件) {
  ...
  ( umask 077 && \mkdir "${_shunit_tmpDir_}" ) || \                # 创建临时文件夹
...
# Args:
#   None
_shunit_mktempFunc(构建几个函数名对应文件) {
...
_shunit_cleanup(清除注册信号) {
  ...
  # Do our work.
  \rm -fr "${__shunit_tmpDir}" # 删除临时文件夹及运行时内容
...
_shunit_configureColor(根据用户喜好配置输出颜色) {
...
# Args:
#   None
_shunit_execSuite(执行测试主函数) {
  for _shunit_test_ in ${__shunit_suite}; do # __shunit_suite为字符串集合
    __shunit_testSuccess=${SHUNIT_TRUE}
    ...
    eval "${_shunit_test_}" # 为何通过eval执行test
...
# Args:
#   None
_shunit_assertPass(统计成功次数和统计总测试次数) {
...
_shunit_assertFail(统计失败次数和统计总测试次数) {
...
# Args:
#   None
_shunit_assertSkip(统计忽略次数和统计总测试次数) {
...
#
# Args:
#   c: string: unescaped character     -- rm *  # delete all files in the current directory
#   s: string: to escape character in  -- rm \* # delete the file named *
# Returns:
#   string: with escaped character(s)
_shunit_escapeCharInStr(实现字符转义) {
...
# Returns:
#   string: with escaped character(s)
_shunit_escapeCharactersInString(实现字符串转义) {
...
_shunit_extractTestFunctions(返回测试函数名序列) {
  _shunit_script_=$1
...
if \[ $# -eq 0 ]; then
  __shunit_script=${__SHUNIT_PARENT}         # $0 为执行脚本名称
  __shunit_mode=${__SHUNIT_MODE_SOURCED}     # 导入模式
else
  __shunit_script=$1
  \[ -r "${__shunit_script}" ] || \
      _shunit_fatal "unable to read from ${__shunit_script}"
  __shunit_mode=${__SHUNIT_MODE_STANDALONE}  # 独立运行模式
fi

# Create a temporary storage location.
__shunit_tmpDir=`_shunit_mktempDir`         # 创建临时文件夹
...
if \[ "${__shunit_mode}" = "${__SHUNIT_MODE_STANDALONE}" ]; then # 独立运行模式
