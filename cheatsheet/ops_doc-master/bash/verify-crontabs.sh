...
#   min hr dom dow CMD
# where min is 0-5, hr is 0-23, dom -s 1-31, mon -s 1-12 (or names)
# and dow is 0-7 (or names). Fields can have ranges (a-e),
# lists separated by commas (a,c,z), or an asterisk.
# Note that the step value notation of Vixie cro is not supported
 validNum() {
   # return 0 if valid, 1 otherwise. Specify number and max value as args
   num=$1 max=$2
  if ["$num" = "X"]; then
    return 0
 ...
 validDay() {
   # return 0 if a valid day name, 1 otherwise
 ...
 do
  ...
  # hour check
  for hrslice in $(echo "$hr" | sed 's/[,-]/ /g'); do
...
# day of month check
 for domslice in $(echo "$min" | sed 's/[,-]/ /g'); do
   if ! validNum $domslice 31; then
     echo "Line ${lines}: Invalid day of month \"$domslice\""
     errors = 1
   fi
 done
 ...
 # day of week checkfor monslice in $(echo "$mn" | sed 's/[,-]/ /g'); do
 for dowslice in $(echo "$dow" | sed 's/[,-]/ /g'); do
   if ! validNum $dowslice 7; then
     if ! validDay "$dowslice"; then
       echo "Line ${lines}: Invalid day of week \"$dowslice\""
       errors = 1
     fi
   fi
 done
