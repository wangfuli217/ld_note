...
1. logrotate是作为linux系统日志的管理工具存在。他可以轮换，压缩，邮件系统日志文件。
   logrotate是个十分有用的工具，它可以自动对日志进行截断（或轮循）、压缩以及删除旧的日志文件。
2. 默认的logrotate被加入cron的/etc/cron.daily中作为每日任务执行。
3. /etc/logrotate.conf为其默认配置文件指定每个日志文件的默认规则。
4. /etc/logrotate.d/* 为/etc/logrotate.conf默认包含目录其中文件也会被logrotate读取。指明每个日志文件的特定规则。
5. /usr/sbin/logrotate -f /etc/logrotate.d/nginx  // 未到时间或者未到切割条件，强制切割
6. /usr/sbin/logrotate -d -f /etc/logrotate.d/nginx // 输出切割debug信息
...
第13行include /etc/logrotate.d 告诉logrotate 读入存放在/etc/logrotate.d 目录中的日志转储参数，
当系统中安装了RPM 软件包时，使用include 选项十分有用。RPM 软件包的日志转储参数一般存放在/etc/logrotate.d 目录。
...
prerotate 命令指定转储以前的动作/usr/bin/chattr -a 去掉/var/log/messages文件的"只追加"属性endscript 
结束prerotate 部分的脚本postrotate 指定转储后的动作
