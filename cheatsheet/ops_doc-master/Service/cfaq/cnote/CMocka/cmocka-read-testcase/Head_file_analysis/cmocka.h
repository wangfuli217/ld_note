...
/*********************************/
#ifdef _WIN32
# ifdef _MSC_VER                      /*_MSC_VER是微软公司推出的C/C++编译器在ANSI/ISO C99标准之外扩展的宏定义，
                                       用来定义当前微软公司自己的编译器的主版本*/
...
#  if _MSC_VER < 1500                  /*MS VC++ 9.0 _MSC_VER = 1500(VisualStudio 2008)*/
#   ifdef __cplusplus
extern "C" {
#   endif   /* __cplusplus */
...
/* If __WORDSIZE is not set, try to figure it out and default to 32 bit. 
   ...
   __WORDSIZE： <bits/wordsize.h> 中定义的字的位数的宏，64位平台上值为64，32位平台上值为32;  
 
   __ILP32__ :  C数据类型为32位的int、long、pointer
   
   __x86_64__是Intel的64位CPU，与x64,AMD64是一样的含义,兼容X86-32架构;
...
#ifndef __WORDSIZE
# if defined(__x86_64__) && !defined(__ILP32__)  //定义了64位的平台，没有定义32位的数据类型
 ...
 */
typedef uintmax_t LargestIntegralType;      /*最大的整数类型*/

#else /* DOXGEN */ 
 
#ifndef LargestIntegralType
# if __WORDSIZE == 64 && !defined(_WIN64)         64位系统下，但是编译环境不是x64
...
/* 
  Printf format used to display LargestIntegralType as a hexidecimal(十六进制). 

  "0x%I64x"：0x表16进制
             I64表示64位int值（I32表示32位int值，i必须大写）；
             x表示用十六进制形式来显示；
	       如： printf("0x%x",5);就输出0x5；
 
 "%#lx"  :  %是格式控制符；
            l:表示长整型 long；
			x:表示输出十六进制 
					
 %#llx ：输出16进制的 long long ；
 
 _win32: 用来判断编译环境是不是window环境(_WIN32变量是windows系统特有的一个宏)； 
 _WIN64：用来判断编译环境是x86还是x64;
 32位系统，只定义了 _WIN32,没有定义 _WIN64; 64位系统，定义了 _WIN32 和 _WIN64
 
*/
#ifndef LargestIntegralTypePrintfFormat
# ifdef _WIN32                                         //移植代码的时候判断平台环境是不是windows
#  define LargestIntegralTypePrintfFormat "0x%I64x"
# else
#  if __WORDSIZE == 64
#   define LargestIntegralTypePrintfFormat "%#lx"     //如果是64位平台，输出long型即可达到8字节
#  else
#   define LargestIntegralTypePrintfFormat "%#llx"    //如果是32位平台，需要输出long long才能达到8字节
#  endif
# endif /* _WIN32 */
#endif  /* LargestIntegralTypePrintfFormat */

/* Printf format used to display LargestIntegralType as a decimal（十进制）. */
...
/* Perform a cast of a pointer to LargestIntegralType */             /*指针类型转换成LargestIntegralType类型*/
#define cast_ptr_to_largest_integral_type(value) \
cast_to_largest_integral_type(cast_to_pointer_integral_type(value))

/* GCC have printf type attribute check. */                          /*GCC的printf类型属性检查*/
#ifdef __GNUC__ 
...
#ifdef DOXYGEN
/**
 * @brief Retrieve a typed return value of the current function. 获取当前函数返回值的类型
 *
 * The value would be casted to type internally to avoid having the 值会进行内部转换以避免调用者手动转换。
 * caller to do the cast manually.
 *
 * @param[in]  #type  The expected type of the return value         /* 输入参数的类型*/
 *
 * @return The value which was stored to return by this function.
 *
 * @code
 *   int param;
 *   param = mock_type(int);    //使用范例
 * @endcode
 *
 * @see will_return()
 * @see mock()
 ...
 *
 * This function needs to be called in the mock object.  这个函数需要在模拟对象中被调用。
 *
 * @param[in]  #parameter  The parameter to check.
 */
void check_expected(#parameter);  /*判断一个函数的参数是否正确*/ 
#else
 ...
 *
 * The function prints an error message to standard error and terminates the
 * test by calling fail() if the pointer is non-NULL. （这里应该是NULL）
 *
 ...
 * test failure is signalled.                                                     //这一段不是很明白
 *
 * @{
 */
 ...
 *     assert(value);   判断value，如果值为0（假），调用abort终止运行程序。
 *     (*value) ++;
 * }
 * @endcode
 *
...
extern jmp_buf global_expect_assert_env;                      /*jmp_buf在C语言标准库 <setjmp.h> 中定义,所以该头文件要加在<cmocka.h>前*/
