...
void test_merge_sort_on_null_array() {
  long int* array = NULL;
  merge_sort((void**) array, 0, compare_long_int);

  TEST_ASSERT(1);
}

void test_merge_sort_on_full_array() {
  ...
  merge_sort((void**) array, 5, compare_long_int);
...
void test_merge_sort_on_full_ptr_array() {
  ...
  merge_sort((void**) array, 5, compare_int_ptr);
  for(int i=0; i<4; ++i) {
    TEST_ASSERT_TRUE(*array[i] <= *array[i+1]);
  }
...
void test_merge_sort_on_full_ptr_array_contratio() {
  // this is the way the sorting api is really intended to be used. The
  // api does not really support raw types. Only pointers.
  int* array[10];
  array[0] = new_int(10);
  ...
  merge_sort((void**) array, 10, compare_int_ptr);
  for(int i=0; i<9; ++i) {
    TEST_ASSERT_TRUE(*array[i] <= *array[i+1]);
  }
...
void test_merge_sort_on_full_ptr_array_contratio_milioni() {
  // this is the way the sorting api is really intended to be used. The
  // api does not really support raw types. Only pointers.
  int size = 20000000;
  ...
  int i;
  for (i=0; i<size; i++) {
    intArray[i] = new_int(size-i);
  }

  puts("Iniziato");

  merge_sort((void**) intArray, size, compare_int_ptr);

  puts("Testo");

  for (i=0; i<size-1; i++) {
    TEST_ASSERT_TRUE(*intArray[i] <= *intArray[i+1]);
  }
