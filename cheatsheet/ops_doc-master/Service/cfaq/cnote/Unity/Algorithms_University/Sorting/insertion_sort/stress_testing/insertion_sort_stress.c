...
#include "insertion_sort_stress.h"
#include "test.h"
#include "file.h"
...
void test_insertion_sort_on_file_val(void* param) {
  pEntry* aEntry = ((struct param*) param)->aEntry;
  size_t sizeAEntry = ((struct param*) param)->sizeAEntry;

  insertion_sort((void**) aEntry, sizeAEntry, compare_entry_val);
...
void test_insertion_sort_on_file_str(void* param) {
  pEntry* aEntry = ((struct param*) param)->aEntry;
  size_t sizeAEntry = ((struct param*) param)->sizeAEntry;

  insertion_sort((void**) aEntry, sizeAEntry, compare_entry_str);
...
void test_insertion_sort_on_file_dec(void* param) {
  pEntry* aEntry = ((struct param*) param)->aEntry;
  size_t sizeAEntry = ((struct param*) param)->sizeAEntry;

  insertion_sort((void**) aEntry, sizeAEntry, compare_entry_decimal);
...
bool parseArgs(char* opt, TestFunction *test_func) {
  if (!strcmp(opt, "int"))
    *test_func = test_insertion_sort_on_file_val;
  else if (!strcmp(opt, "str"))
    *test_func = test_insertion_sort_on_file_str;
  else if (!strcmp(opt, "float"))
    *test_func = test_insertion_sort_on_file_dec;
  else {
    puts("Wrong pars!");
    return false;
  }
