...
void test_heap_sort_on_full_array() {

  assert(sizeof(void*) == sizeof(long));
  long int array[5] = { 11, 4, 1, 8, 10 };
  heap_sort((void**) array, 5, compare_long_int);
...
void test_heap_sort_on_full_ptr_array() {
  ...
  heap_sort((void**) array, 5, compare_int_ptr);
  for(int i=0; i<4; ++i) {
    TEST_ASSERT_TRUE(*array[i] <= *array[i+1]);
  }
...
void test_heap_sort_on_full_ptr_array_descending() {
  ...
  heap_sort((void**) array, 10, compare_int_ptr);
  for(int i=0; i<9; ++i) {
    TEST_ASSERT_TRUE(*array[i] <= *array[i+1]);
  }
