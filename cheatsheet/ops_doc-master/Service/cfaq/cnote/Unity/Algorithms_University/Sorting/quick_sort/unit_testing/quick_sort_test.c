...
void test_quick_sort_on_null_array() {
  long int* array = NULL;
  quick_sort((void**) array, 0, compare_long_int);
...
void test_quick_sort_on_full_array() {
  ...
  quick_sort((void**) array, 5, compare_long_int);
...
void test_quick_sort_on_full_ptr_array() {
  ...
  quick_sort((void**) array, 5, compare_int_ptr);
  for(int i=0; i<4; ++i) {
    TEST_ASSERT_TRUE(*array[i] <= *array[i+1]);
  }
...
void test_quick_sort_on_full_ptr_array_contratio() {
  // this is the way the sorting api is really intended to be used. The
  // api does not really support raw types. Only pointers.

  int size = 200000;

  int** array = (int**) malloc(sizeof(void*)*size);
  ...
  quick_sort((void**) array, size, compare_int_ptr);
