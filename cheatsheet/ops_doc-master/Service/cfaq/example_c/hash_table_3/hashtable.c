...
hash_table_t * hash_table_new(hash_table_mode_t mode)
{
    ...
    table->key_num = 128;
    table->key_ratio = 4;
    table->store_house = (hash_table_element_t **) calloc(table->key_num, sizeof(hash_table_element_t *)); /* [key][n_value] */
...
{
    if ((table->key_count / table->key_num) >= table->key_ratio) { //判断key的使用率，如果过高需要进行扩展
    ...
    size_t hash = HASH(key, key_len); /* 获取key的hash值 */
    ...
    if (table->mode == MODE_COPY) /* hashtable 的类型 为 深度copy*/
    {
        LOG("Adding a key-value pair to the hash table with hash -> %d, in COPY MODE", (int)hash);
        element->key = malloc(key_len); /* 申请Key存储空间 */
        element->value = malloc(value_len); /* 申请value存储空间 */
        if (element->key && element->value) { /* 复制 */
    ...
    } else if (table->mode == MODE_VALUEREF) {/* hashtable 类型为 引用 */
        ...
        element->value = value; /* 指针赋值 */
    }
    else if (table->mode == MODE_ALLREF) /* 全引用模式 */
    {
    ...
    if (!table->store_house[hash]) {
        ...
        while(temp->next) {
            while(temp->next && temp->next->key_len != key_len) { /* 查询 key_len 相等的node */
                temp = temp->next;
            }
 ...
 */
void* 
hash_table_lookup(hash_table_t * table, void * key, size_t key_len)
{
    size_t hash = HASH(key, key_len);
