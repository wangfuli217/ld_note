...
#define HEAP_HEADER_SIZE ((sizeof(struct bpa_heap) + ALIGN_SIZE - 1) / ALIGN_SIZE) /* 对齐后的长度 */
#define HEAP_SLICE(H, off) (struct slice *)((char *)H + off * ALIGN_SIZE) /* 获取内存实际的片 */
#define SLICE_ID(S) ((id *)((uintptr_t)(S) + S->sz * ALIGN_SIZE - sizeof(id)))
#define MAX_SLOT 0x10000 /* 64k == 65536 */
...
bpa_init(void *buffer, size_t sz) {
	...
	H->sz = sz;
	H->offset = HEAP_HEADER_SIZE; /* 可用空间从这里开始 */
	H->collect = 0;
	H->cid = 0;
...
bpa_create(struct bpa_heap *H, size_t sz) {
	pointer_t rsz = (sz + sizeof(struct slice) + sizeof(id) + ALIGN_SIZE - 1) / ALIGN_SIZE; /* 实际占用的空间 */
	if (H->offset + rsz > H->sz)
		return 0;
	int slot = alloc_slot(H); /* 选一个空闲slot */
	if (slot < 0) {
		return 0;
	}
	id cid = H->cid;
