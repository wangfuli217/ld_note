uv(type){ https://github.com/thlorenz/libuv-dox/blob/master/types.md

句柄和请求
loop # 全局生命期
    uv_loop_t

requests # 调用接口和回调接口之间生命期
    uv_req_t
...
buffers # 调用接口和回调接口之间生命期
    uv_buf_t

handles # 对象在open和close之间生命期
    uv_handle_t 
    streams # 使用uv_read2_start(uv_read2_cb)和uv_read_start(uv_read_cb)进行数据读取。使用uv_write(uv_write_cb)进行数据写入
...
struct uv_loop_s {
  ...
  // uv-common.c完成对handle_queue的操作
  void* handle_queue[2];  # uv_handle_t链表{uv_stream_t{uv_tcp_t uv_tty_t uv_pipe_t},uv_udp_t uv_poll_t uv_prepare_t uv_idle_t uv_async_t uv_timer_t uv_process_t}
  void* active_reqs[2];   # 
