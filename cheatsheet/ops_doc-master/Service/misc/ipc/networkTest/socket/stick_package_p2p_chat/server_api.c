...
//	char recvbuf[MAX_BUF];
	...
	{
		memset(&pack,0,MAX_BUF+sizeof(size_t));
		rslt = readn(sock,&pack.len,sizeof(size_t));
		if(rslt  < 4)
		{
		...
		rslt = readn(sock,pack.buf,pack.len);
		if(rslt  < pack.len)
		{
			printf("client closed\n");
			break;
