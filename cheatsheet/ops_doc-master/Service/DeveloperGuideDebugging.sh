...
show copying    # 显示gdb版权相关信息
show warranty　 # 显示gdb版权相关信息
gdb -q          # 启动时不显示提示信息
set confirm off # 退出时不显示提示信息
set pagination off 或者 set height 0 # 输出信息多时不会暂停输出

info functions       # 列出函数的名字
info functions thre* # 列出函数的名字
step # 是否进入带调试信息的函数    进入
next # 是否进入带调试信息的函数    不进入
set step-mode on # 进入不带调试信息的函数
return 或者 finish # 退出正在调试的函数
...
info break [list...]
list... 用来指定若干个断点的编号（可省略），可以是 2， 1-3， 2 5 等。 # list... 同 info breakpoints
...
undisplay           # 取消某些表达式在程序暂停时的自动显示。  delete display
disable display     # 禁用某些表达式在程序暂停时的自动显示。  
enable display      # 启用某些表达式在程序暂停时的自动显示。
}
