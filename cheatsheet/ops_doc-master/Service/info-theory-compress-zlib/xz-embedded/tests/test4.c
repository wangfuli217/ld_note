...
int main(int argc, char *argv[]) {
  ...
  olen = 5; /* make it too small */

 retry:
  tmp = realloc(out, olen);
  if (tmp == NULL) {
  ...
  xc = xz_dec_run(d, &b);
  if (xc == XZ_BUF_ERROR) { /* need more outbut buffer */
    printf("growing buffer to retry\n");
    olen *= 2;
    if (failsafe++ > 10) goto done;
    goto retry;
  }
