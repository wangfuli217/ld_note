...
void add_alias(char *fakename, char *realname, int type)
{
    ...
    if (old) {
        while (old->next) {
            if (!strcmp(fakename, old->fakename)) 
                return;
            old = old->next;
        }
    }
...
int translate_uri(request * req)
{
    ...
    if (strcmp(CGI_MIME_TYPE, get_mime_type(buffer)) == 0) { 
#ifdef FASCIST_LOGGING
        log_error_time();
...
int init_script_alias(request * req, alias * current1, int uri_len)
{
    ...
    if (c == '\0') {
         ...
         */
        req->script_name[strlen(req->script_name) - path_len] = '\0'; 
