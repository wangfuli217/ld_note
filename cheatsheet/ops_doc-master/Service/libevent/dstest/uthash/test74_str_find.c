...
int main (void) {
    ...
    V_FindPos = 0;
    do {
        V_FindPos = utstring_find (s, V_FindPos, utstring_body (t), utstring_len (t));
        printf ("utstring find=%ld\n", V_FindPos);
        if (V_FindPos >= 0) {
            V_FindPos++;
            V_FindCnt++;
        }
    ...
    V_FindPos = -1;
    do {
        V_FindPos = utstring_findR (s, V_FindPos, utstring_body (t), utstring_len (t));
        printf ("utstring find=%ld\n", V_FindPos);
        if (V_FindPos >= 0) {
            V_FindPos++;
            V_FindCnt++;
