...
int main (int argc, char *argv[]) {
    ...
    printf ("keylen:%d offsetof(struct my_event, event_code):%lu sizeof(char):%lu offsetof(struct my_event, is):%lu\n",
            keylen, offsetof (struct my_event, event_code), sizeof (char), offsetof (struct my_event, is));
    for (i = 0; i < 10; i++) {
