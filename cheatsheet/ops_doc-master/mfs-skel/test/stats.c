...
void stats_show_all(uint8_t **buff,uint32_t *leng) {
    uint32_t rl;
    pthread_mutex_lock(&glock);
...
int main(void) {
    ...
    n1 = stats_get_subnode(NULL,"ala", 0, 0);
    s = stats_get_counterptr(n1);
    (*s) += 200;
    n2 = stats_get_subnode(n1,"ma", 0, 1);
    n3 = stats_get_subnode(n2,"kota", 0, 1);
    s = stats_get_counterptr(n3);
    (*s) += 15;
    n3 = stats_get_subnode(n2,"psa",0 ,1);
    s = stats_get_counterptr(n3);
    (*s) = 10;
    n1 = stats_get_subnode(NULL,"krowa",0,1);
    n1 = stats_get_subnode(NULL,"pikus", 0, 1);
    n2 = stats_get_subnode(n1,"barbapapa",1, 1);
    s = stats_get_counterptr(n3);
