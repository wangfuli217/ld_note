...
local function dhcp_leases_common(family)
	...
	local leasefile = "/tmp/dhcp.leases" -- default
-- 1522422515 00:22:64:64:db:8b 192.168.110.159 wangfuli-HP-541 xx:......
-- expire     mac               ip              name            duid
	uci:foreach("dhcp", "dnsmasq",
		function(s)
			if s.leasefile and nfs.access(s.leasefile) then
				leasefile = s.leasefile  -- configuration
				return false
			end
		end)
		...
		while true do
			...
			if not ln then
				...
				if ts and mac and ip and name and duid then
					if family == 4 and not ip:match(":") then -- ipv4地址不是以:区分的
						rv[#rv+1] = {
					...
					elseif family == 6 and ip:match(":") then -- ipv6地址是以:区分的
						rv[#rv+1] = {
							expires  = (expire ~= 0) and os.difftime(expire, os.time()),
							ip6addr  = ip,
							duid     = (duid ~= "*") and duid,
...
-- 主要功能是:路由器通告，转发路由器通告，DHCPv6和邻居发现协议，dhcpv4客户端和服务器端功能没有实现
