...
s.anonymous = true -- 当执行TypedSection()函数的时候就会判断这个和下面的选项
s.addremove = false

s:tab("general", translate("General Settings")) -- 定义一个tab给s，调用abstractSection : tab(tab,title,desc)函数，其中
...
--       childs      = { }  
--   }  
-- 将tab给了tab_names，tab的各个参数给了tabs数组。
