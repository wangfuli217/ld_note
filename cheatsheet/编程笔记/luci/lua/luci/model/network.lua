...
-- 虚拟接口表 IFACE_PATTERNS_VIRTUAL 支持模式匹配
...
-- 忽略接口表 IFACE_PATTERNS_IGNORE 支持模式匹配
...
-- 初始化 _interfaces, _bridge, _switch, _tunnel, _swtopo
--        _ubusnetcache, _ubusdevcache, _ubuswificache
--        _uci
function init(cursor)
	...
	for n, i in ipairs(nxo.getifaddrs()) do
		local name = i.name:match("[^:]+") -- 过滤掉名字开头部分的:开头
		...
		if _tunnel[name] or not (_iface_ignore(name) or _iface_virtual(name)) then
			_interfaces[name] = _interfaces[name] or {
				idx      = i.ifindex or n, -- 接口序号
				name     = name,    -- 接口名称
				rawname  = i.name,  -- 接口raw名称
				flags    = { },     -- 接口标识
				ipaddrs  = { },     -- ip地址
				ip6addrs = { }      -- ipv6地址
			}
...
-- pointtopoint = false
	...
	local b, l -- 返回一个桥表，name桥名 id桥id stp桥stp ifnames桥接口 ifnames{}.bridge 关联桥
