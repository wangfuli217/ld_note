...
九、HTTP 方法
    ...
    缺省情况下，一个路由只回应 GET 请求，但是可以使用不同方法。
    ...
    HEAD
        ...
        一个应用应该像接受到一个 GET 请求一样运行，但是不传递实际的内容。

    POST
        浏览器告诉服务器想要向 URL 发表 一些新的信息，服务器必须确保数据被保存好 且只保存了一次。
        HTML 表单实际上就是使用这个访求向服务器传送数据的。

    PUT
        与 POST 方法类似，不同的是服务器可能触发多次储存过程而把旧的值覆盖掉。
        你 可能会问这样做有什么用？这样做是有原因的。
        假设在传输过程中连接丢失的情况 下，一个处于浏览器和服务器之间的系统可以在不中断的情况下安全地接收第二次 请求。
        在这种情况下，使用 POST 方法就无法做到了，因为它只被触发一次。

    DELETE
        删除给定位置的信息。

    OPTIONS
        为客户端提供一个查询 URL 支持哪些方法的捷径。
        有趣的是在 HTML4 和 XHTML1 中，表单只能使用 GET 和 POST 方法。
        但是 JavaScript 和未来的 HTML 标准中可以使用其他的方法。
        此外， HTTP 近来已经变得相当 流行，浏览器不再只是唯一使用 HTTP 的客户端。比如许多版本控制系统也使用 HTTP 。
...
<input type=hidden>      隐藏域，存储不必显示但需要提交的信息。
...
<kbd>                以固定宽度和字体渲染文本。
<label>              为页面上的其它元素指定标签。(火狐认为是不可修改的，但IE可以修改)
...
<style></style>      代表了给定元素所有可能的内嵌样式的当前设置。指定页面的样式表。
<styleSheet>         代表了文档中单一的样式表。
<sub>                指定内含文本要以下标的形式显示，通常比当前字体稍小。 (subscript['sʌbskript]的缩写)
...
Banner               使用 event 对象获取高级流复位向器(ASX)文件中项目的 Banner 内容。
...
MoreInfo             通过 event 对象获取高级流复位向器(ASX)文件中项目横幅的 MoreInfo 内容。
