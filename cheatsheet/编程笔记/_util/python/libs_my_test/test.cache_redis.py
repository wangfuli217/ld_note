...
class TestRedisCache(unittest.TestCase):
    ...
    def test_fn_timeout(self):
        cache.init(fn_timeout = 1)
        logging.warn(u'fn 写上缓存时间的调用方式,缓存1秒')
    ...
    def test_fn_name(self):
        '''区分被 装饰器(Decorator) 包装的函数(单纯依靠 method.__module__, method.__name__ 是无法区分的),所以必须设置 name 参数'''
