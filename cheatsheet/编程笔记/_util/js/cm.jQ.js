...
/**
 * 获取事件的 event
 * @return {Object} 事件的 event
 */
c$.getEvent = function() {
    if (jQuery.browser.msie) return window.event; // IE
 ...
 * jQuery 额外添加 start
 * --------------------------------------------------------------- */
//把对象或者对象下的子元素序列化成数组:[ {name:'元素的name的值', value:'元素的value的值'}, {name:'name', value:'value'}]
//需要对值encodeURI的元素,需加上 encodeurl='true' 属性
$.fn.serializeOpinion = function () {
...
$.fn.clearInputs = function (isClearhidden) {
    ...
    if (isClearhidden) $(":hidden", this).val("");
};
...
$.fn.checkBoxSelect = function (name, checked) {
    name = name ? ("[name=" + name + "]") : '';
    var checkBoxs = $(":checkbox" + name, this); // 选取 checkbox元素
...
//all: 为true时,需要所有都选中时才返回true；为false时只要有任意一个选中即返回true, 默认为false
...
c$.setFocus = function(element) {
    //element.focus();
    setTimeout(function(){try { jQuery(element).focus().select(); } catch (e){}}, 10);
    return this;
};
