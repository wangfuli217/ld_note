...
static int handle_script (lua_State *L, char **argv, int n) {
  ...
  // local f = load(filename) f()
  // load会将文件chunk编译成一个function，然后我们就可以对它调用。
  // 如果我们详细看lua文档的话，这个函数可以带有upvalues，也就是这个函数其实是一个闭包（closure）。
  // 每个运行时期的可调用的lua函数都是闭包。
...
int main (int argc, char **argv) {
  ...
  s.argc = argc;
  s.argv = argv;
/* main函数创建了lua_State之后就按照调用C导出给lua函数的方式调用了pmain函数。
pmain函数中通过lua栈获取到命令行的argc和argv参数之后，对参数进行分析后，
主要可以分为两个分支，一个处理交互命令行，一个处理文件。dotty出来交互命令行，handle_script处理lua文件。
*/
