...
m.title	= [[</a><a href="javascript:alert(']]
		...
		.. [[\n\n]]
	.. [[')">]]
	.. translate("<font color=\"blue\"><strong>Dynamic DNS</strong></font>")

m.description = translate("<font color=\"Red\"><strong>建议来源配置为：端口，端口配置为pppoe-wan，否则在多拨或翻墙情况下，无法获取到正确的IP地址。另外固件已默认停止ddns服务，需要先开启ddns服务，可以点击下面的“强启”按钮。ddns在本固件一共有三种不同的设置方法，还有石像鬼和脚本的处理方法。</strong></font>")

m.on_after_commit = function(self)
	if self.changed then	-- changes ?
		if SYS.init.enabled("ddns") then	-- ddns service enabled, restart all
			os.execute("/etc/init.d/ddns enable && /etc/init.d/ddns restart")
...
if show_hints or need_update or not SYS.init.enabled("ddns") then
	...
	if show_hints then
		...
		dv.titleref = DISP.build_url("admin", "advance", "ddns", "hints")
		dv.rawhtml  = true
		dv.title = bold_on ..
...
function dom.set_two(self, section)
	...
	local command = self.map:get(section, "domain_script") or ""
	if command == "" then
	...
	command = [[/usr/lib/ddns/dynamic_dns_lucihelper.sh]]
