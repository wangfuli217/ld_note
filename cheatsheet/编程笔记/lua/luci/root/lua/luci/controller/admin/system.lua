...
-- Cbi函数返回四个值，type,config,model,target
	...
	entry({"admin", "system", "admin"}, cbi("admin_system/admin"), _("Administration"), 2) -- cbi可同时返回多个Map

	if fs.access("/bin/opkg") then
		entry({"admin", "system", "packages"}, post_on({ exec = "1" }, "action_packages"), _("Software"), 10) -- 够复杂，难理解
	...
	entry({"admin", "system", "flashops", "reset"}, post("action_reset"))    -- luci\view\admin_system\flashops.htm
	entry({"admin", "system", "flashops", "backup"}, post("action_backup"))  -- 
	entry({"admin", "system", "flashops", "backupfiles"}, form("admin_system/backupfiles")) -- luci\view\admin_system\flashops.htm

	-- call() instead of post() due to upload handling!
	entry({"admin", "system", "flashops", "restore"}, call("action_restore")) -- luci\view\admin_system\flashops.htm
	entry({"admin", "system", "flashops", "sysupgrade"}, call("action_sysupgrade")) -- luci\view\admin_system\flashops.htm

	entry({"admin", "system", "reboot"}, template("admin_system/reboot"), _("Reboot"), 90) -- 页面显示 ->
	entry({"admin", "system", "reboot", "call"}, post("action_reboot")) -- 页面请求处理
end
...
function action_sysupgrade()
	...
	if step == 1 then           -- 可以升级走此流程
		if image_supported(image_tmp) then      -- 通过文件头检查文件是否正确 27051956 见/lib/upgrade/platform.sh文件中
	...
	--
	elseif step == 2 then  -- 进行升级处理
		local keep = (http.formvalue("keep") == "1") and "" or "-n"
		luci.template.render("admin_system/applyreboot", { -- 处理完请求后调用
			title = luci.i18n.translate("Flashing..."),
			msg   = luci.i18n.translate("The system is flashing now.<br /> DO NOT POWER OFF THE DEVICE!<br /> Wait a few minutes before you try to reconnect. It might be necessary to renew the address of your computer to reach the device again, depending on your settings."),
			addr  = (#keep > 0) and "192.168.1.1" or nil
		}) -- 升级页面
		fork_exec("sleep 1; killall dropbear uhttpd; sleep 1; /sbin/sysupgrade %s %q" %{ keep, image_tmp }) -- 升级处理
	end
end
