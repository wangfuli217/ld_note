...
s.anonymous = true
s.sortable  = true
s.extedit   = ds.build_url("admin/network/firewall/forwards/%s") -- tblsection.htm 中参数，用于edit按钮时跳转
s.template_addremove = "firewall/cbi_addforward"

function s.create(self, section) -- 重载构建函数
	local n = m:formvalue("_newfwd.name")   -- 从cbi_addforward.htm获得的数据
	...
	if p == "other" or (p and a) then
		created = TypedSection.create(self, section) -- 调用上层构建函数
-- 配置/etc/config/firewall文件
...
function s.parse(self, ...) -- 重载section解析函数
	TypedSection.parse(self, ...) -- 调用section解析函数
	if created then -- 如果不是tcp和udp协议的情况下
		m.uci:save("firewall")
...
function s.filter(self, sid) -- 过滤SNAT
