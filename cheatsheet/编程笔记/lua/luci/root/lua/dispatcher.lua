...
function dispatch(request)
	...
	track.dependent = (track.dependent ~= false) -- 该节点没有父节点，不允许访问
	...
	log.print(track.sysauth, "xxxxxxxx")
...
-- 将getcookie和sysauth属性赋给sess  
        ...
        log.print("def", def)
		log.print("accs",accs)
		log.print("verifytoken", verifytoken)
...
function _create_node(path)
	...
	if not c then
		...
		c = {nodes={}, auto=true}  -- self.nodes() initilize
		-- the node is "in request" if the request path matches
		-- at least up to the length of the node path
		if parent.inreq and context.path[#path+1] == last then
		  c.inreq = true
		end
		parent.nodes[last] = c    -- set parent.nodes{} 
...
local function _cbi(self, ...)
	...
	for i, res in ipairs(maps) do
		...
		if res.pageaction == false then
			pageaction = false -- 对cbi/footer.htm中的按钮进行控制，总开关
		end
