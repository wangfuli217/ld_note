...
*/
// 新的打开函数luaopen_array必须创建元表，并将它赋予__index字段，然后在元表中注册所有的方法，
// 最后创建并填充array table
...
// 其中用到了luaL_register的另一个特性。在第一次调用中，以NULL作为库名，
// luaL_register不会创建任何用于存储函数的table，而是以栈顶的table作为存储函数的table。
// 在本例中，栈顶table就是元表本身，因此luaL_register会将所有的方法放入其中。
    luaL_register(L, NULL, arraylib_m);
// 第二次调用luaL_register则提供了一个库名，它就根据此名（array）创建了一个新table，
// 并将指定的函数注册在这个table中
