...
int nixio__check_mode(lua_State *L, int idx, int def) {
	if (lua_isnoneornil(L, idx) && def > 0) { // nil TNONE 则默认值为def 0666
		return def;
	} else if (lua_isstring(L, idx) && lua_objlen(L, idx) == 9) { // 是字符串，且字符串长度等于9  rwxrwxrwx
		int mode = 0;
	...
	} else if (lua_isnumber(L, idx)) { // integer  十进制转换为八进制
...
// rwx x->[sS]  04100 s 04000 S 00100 x
// rwx x->[sS]  02010 s 02000 S 02010 x
// rwx x->[tT]   01001 t 01000 T 02010 x
...
static int nixio_dir(lua_State *L) {
	...
	DIR **dirp = lua_newuserdata(L, sizeof(DIR *)); // [D]

	*dirp = opendir(path);
	if (!*dirp) {
		return nixio__perror(L);
	} else {
		luaL_getmetatable(L, NIXIO_DIR_META); // [D] [nixio.dir]
		lua_setmetatable(L, -2); // [D]
		// void lua_pushcclosure(L, CF cf, int n);
...
int nixio__push_stat(lua_State *L, nixio_stat_t *buf) {
	lua_createtable(L, 0, 15); // [T]

	lua_pushinteger(L, buf->st_dev); // [T] [st_dev]
	lua_setfield(L, -2, "dev"); // [T] 

	lua_pushinteger(L, buf->st_ino); // [T] [st_ino]
	lua_setfield(L, -2, "ino");// [T] 
	...
	lua_setfield(L, -2, "modedec"); // 10 进制数值

	lua_pushlstring(L, modestr, 9); // rwxrwxrwx
...
static int nixio_chown(lua_State *L) {
	return nixio__pstatus(L,
			!chown(
					...
					lua_isnoneornil(L, 2) ? -1 : nixio__check_user(L, 2),  // -1 uid 不变
					lua_isnoneornil(L, 3) ? -1 : nixio__check_group(L, 3) // -1 gid 不变
			)
	);
}
...
static int nixio_glob(lua_State *L) {
	 ...
	 nixio_glob_t *globres = lua_newuserdata(L, sizeof(nixio_glob_t));  // [u]
	 if (!globres) {
	 ...
	 if (globstat == GLOB_NOMATCH) {
		 lua_pushcfunction(L, nixio__nulliter); // 没有匹配项，则调用函数就返回nil
		 lua_pushinteger(L, 0);
	 } else if (globstat) {
		 return nixio__perror(L);
	 } else {
		 luaL_getmetatable(L, NIXIO_GLOB_META); //  [u] [nixio.glob]
		 lua_setmetatable(L, -2); //  [u] 
		 lua_pushcclosure(L, nixio_glob__iter, 1); //  [u] [func]
		 lua_pushinteger(L, globres->gl.gl_pathc); //  [u] [func] [数值]
	 }
	 return 2;
}
...
void nixio_open_fs(lua_State *L) {
	...
	luaL_register(L, NULL, R); // [nixio.socket], [nixio] {} 注册R中的函数到{}表中
	lua_setfield(L, -2, "fs"); // [nixio.socket], [nixio] 设置fs指向栈顶的{}表

	luaL_newmetatable(L, NIXIO_DIR_META); // [nixio.socket], [nixio] [nixio.dir]
	lua_pushcfunction(L, nixio_dir__gc);  // [nixio.socket], [nixio] [nixio.dir] nixio_dir__gc
	lua_setfield(L, -2, "__gc"); // [nixio.socket], [nixio] [nixio.dir] 设置__gc指向nixio_dir__gc
	lua_pop(L, 1); // [nixio.socket], [nixio]
