...
void nixio_open_tls_socket(lua_State *L) {
	...
	luaL_newmetatable(L, NIXIO_TLS_SOCK_META); // [nixio.socket], [nixio], [nixio.tls.sock]
	luaL_register(L, NULL, M); // [nixio.socket], [nixio], [nixio.tls.sock] 将R表中函数注册到nixio.tls.sock中
	lua_pushvalue(L, -1); // [nixio.socket], [nixio], [nixio.tls.sock], [nixio.tls.sock]
	lua_setfield(L, -2, "__index"); // [nixio.socket], [nixio], [nixio.tls.sock] 设置[nixio.tls.sock].__index 指向自身
	lua_setfield(L, -2, "meta_tls_socket"); // [nixio.socket], [nixio] 设置nixio.meta_tls_socket 指向[nixio.tls.sock]
}
