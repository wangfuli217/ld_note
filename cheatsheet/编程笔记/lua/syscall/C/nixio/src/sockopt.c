...
void nixio_open_sockopt(lua_State *L) {
	...
	luaL_register(L, NULL, M); // [nixio.socket], [nixio], [nixio.socket] 将M表中函数注册到nixio.socket中
	lua_pop(L, 1);             // [nixio.socket], [nixio]

	luaL_getmetatable(L, NIXIO_FILE_META); // [nixio.socket], [nixio], [nixio.file]
	lua_pushcfunction(L, nixio_sock_setblocking); // [nixio.socket], [nixio], [nixio.file] function
	lua_setfield(L, -2, "setblocking");           // [nixio.socket], [nixio], [nixio.file] 设置nixio_sock_setblocking调用名称setblockling
	lua_pushcfunction(L, nixio_sock_fileno); // [nixio.socket], [nixio], [nixio.file] function
	lua_setfield(L, -2, "fileno");           // [nixio.socket], [nixio], [nixio.file] 设置nixio_sock_fileno调用名称fileno
	lua_pop(L, 1);
}
