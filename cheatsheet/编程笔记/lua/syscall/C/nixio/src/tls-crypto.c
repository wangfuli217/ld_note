...
void nixio_open_tls_crypto(lua_State *L) {
	luaL_newmetatable(L, NIXIO_CRYPTO_HASH_META); // [nixio.socket], [nixio], [nixio.crypto.hash]
	luaL_register(L, NULL, M); // [nixio.socket], [nixio], [nixio.crypto.hash] 将M中函数注册到nixio.crypto.hash
	lua_pushvalue(L, -1); // [nixio.socket], [nixio], [nixio.crypto.hash], [nixio.crypto.hash] 
	lua_setfield(L, -2, "__index"); // [nixio.socket], [nixio], [nixio.crypto.hash] 设置[nixio.crypto.hash].__index 指向自身
	lua_pop(L, 1); // [nixio.socket], [nixio],

	lua_newtable(L); // [nixio.socket], [nixio], {}
    luaL_register(L, NULL, R); // [nixio.socket], [nixio], {}将R中函数注册到{}

	lua_setfield(L, -2, "crypto"); // [nixio.socket], [nixio] 将nixio.cryto指向{}
}
