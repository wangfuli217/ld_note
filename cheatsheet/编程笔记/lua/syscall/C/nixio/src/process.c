...
int nixio__exec(lua_State *L, int m) {
	...
	if (m == NIXIO_EXECVE) { //table
		luaL_checktype(L, 2, LUA_TTABLE);
		argn = lua_objlen(L, 2) + 1; // table length + 1
	} else {
		argn = lua_gettop(L); //args
	}

	char **args = lua_newuserdata(L, sizeof(char*) * (argn + 1));  //free by lua self
	args[argn] = NULL;
	args[0] = (char *)path;

	if (m == NIXIO_EXECVE) {
		for (i = 1; i < argn; i++) { // number table
// 将table形式的value列表转换成"value" 字符串形式栈缓存
			lua_rawgeti(L, 2, i);    // void lua_rawgeti (lua_State *L, int index, int n);
			arg = lua_tostring(L, -1);
			luaL_argcheck(L, arg, 2, "invalid argument");
			args[i] = (char *)arg;
		}

		if (lua_isnoneornil(L, 3)) { // 可以传递nil值，也可以不传递该值
			execv(path, args);
		} else {
			luaL_checktype(L, 3, LUA_TTABLE); // enviroments
			argn = 0;
			lua_pushnil(L);
			while (lua_next(L, 3)) { // key-value table
				if (!lua_checkstack(L, 1)) {
					lua_settop(L, 0);
					return luaL_error(L, "stack overflow");
				}
// 将table形式的key-value 转换成"key=value"方式的字符串形式
				if (!lua_type(L, -2) != LUA_TSTRING || !lua_isstring(L, -1)) { //key-value 说明 key must be string, value can string or number
			...
			char **env = lua_newuserdata(L, sizeof(char*) * (argn + 1));  //free by lua self
			env[argn] = NULL;
