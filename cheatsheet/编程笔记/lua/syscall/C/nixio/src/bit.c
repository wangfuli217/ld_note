...
void nixio_open_bit(lua_State *L) {
	...
	nixio__pushnumber(L, NIXIO_BIT_BMAX); // [nixio.socket], [nixio] {} 32
	lua_setfield(L, -2, "bits"); // [nixio.socket], [nixio] {} 设置{}中bits值为32
	nixio__pushnumber(L, NIXIO_BIT_NMAX); // [nixio.socket], [nixio] {} 0xffffffff
	lua_setfield(L, -2, "max"); // [nixio.socket], [nixio] {} 设置{}中max值为0xffffffff
	lua_setfield(L, -2, "bit"); // [nixio.socket], [nixio] 设置bit指向栈顶的{}表
}
