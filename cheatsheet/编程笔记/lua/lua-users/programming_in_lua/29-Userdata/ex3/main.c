...
static int tostring(lua_State *L){
    luaL_checkudata(L,1,"Ex.array");
    NumArray *array = (NumArray *)lua_touserdata(L,1);
    luaL_Buffer buffer ;
    luaL_buffinitsize(L,&buffer,array->size);
    for(int i=0;i<array->size;i++){
        luaL_addstring(&buffer,(array->values[I_WORD(i)] & I_BIT(i)) == 0 ? "0" : "1");
    }
